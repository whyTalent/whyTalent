...
其实该问题可以转换为 **Subset Sum** 问题，即可以将这组数看成两部分数字之和 P 和 N，其中 P 使用正号，N 使用负号，有以下推导：
...
因为硬币可以重复使用，因此这是一个**完全背包问题**，而完全背包只需要 ***将 0-1 背包的逆序遍历 dp 数组改为正序遍历即可***。
...
>
> **dp[i]\[j]=Min(dp[i-1]\[j], dp[i]\[j-vi]+wi)**
>
...
> ​      
>
> 分析本题，可以很明确的看出来，这就是一个完全背包的问题，即总金额就是背包容量，硬币就是物品（无限制使用）
>
...
完全背包问题，使用 dp 记录可达成目标的组合数目，即定义 dp[i]\[j] 表示 使用前 i 种硬币组成数量 j 的组合数，则根据是否使用第i枚硬币，可分为以下2种情况：
...
### 3.7.7 [字符串按单词列表拆分](https://leetcode.com/problems/word-break/description/)

**问题**：给定一个字符串 s 和一个字符串字典 wordDict，如果 s 可以被分割成由一个或多个字典单词以空格分隔的字符串序列，则返回 true（PS：字典中的同一个词可能会在分割序列中多次重复使用）。
...
wordDict 中的单词没有使用次数的限制，因此这是一个完全背包问题，同时问题涉及到字典中单词的使用顺序，也就是说**物品必须按一定顺序放入背包**中，例如下面的 dict 就不够组成字符串 "leetcode"：

```
["lee", "tc", "cod"]
```

求解 **顺序的完全背包问题** 时，对**物品的迭代应该放在最里层，对背包的迭代放在外层**，只有这样才能让物品按一定顺序放入背包中。

假设dp[i] 表示范围 [0, i) 内的子串是否可以拆分，注意这里 dp 数组的长度比s串的长度大1，因为需要处理 空串的情况，初始化 dp[0] = true。

假设 j 把 [0, i) 范围内的子串分为了两部分，**[0, j)** 和 **[j, i)**，其中范围 [0, j) 就是 dp[j]，范围 [j, i) 就是 s.substr(j, i-j)，其中 dp[j] 是之前的状态，可以直接取，只需要在字典中查找 s.substr(j, i-j) 是否存在了，如果二者均为 true，将 dp[i] 赋为 true，其次此时不需要再用j去分 [0, i) 范围了，因为 [0, i) 范围已经可以拆分了。
...
func wordBreak(s string, wordDict []string) bool {
    ...
    for i:=1; i<=len(s); i++ {
        ...
        for _, word := range wordDict {
            l_word := len(word)
            if i >= l_word && s[i-l_word:i] == word {
                dp[i] = dp[i] || dp[i-l_word]
            }
        }
    }
...
补充：下面简单介绍词题其它解题思路，方便理解

```java
// Brute Force(递归): T(2^n) && S(n), 使用辅助数组保存任意可能的字符串子串
public class Solution {
...
// Recursion with memoization: T(n^3) && S(n)
public class Solution {
    public boolean wordBreak(String s, List<String> wordDict) {
...
### 3.7.8 [数字组合总数](https://leetcode.com/problems/combination-sum-iv/description/)

**问题**：给定一组不同的整数 nums 和一个目标整数 target，返回加起来等于 target 的可能组合的数量
...
定义 dp[i] 表示目标数为i的解的个数，然后从1遍历到 target，对于每一个数i，遍历 nums 数组，如果 i>=x, dp[i] += dp[i - x]。

> 比如：对于 nums = [1,2,3] / target= 4，当计算 dp[3] 的时候，3可以拆分为 1+x，而x即为 dp[2]，3也可以拆分为 2+x，此时x为 dp[1]，3同样可以拆为 3+x，此时x为 dp[0]，把所有的情况加起来就是组成3的所有情况了
...
### 3.8.1 [具有冷却期的股票交易](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-with-cooldown/description/)

**问题**：给定一个数组 prices，即 prices[i] 是给定股票第 i 天的价格，返回得到股票交易获得的最大利润，其中可以根据需要完成任意数量的交易（即多次买入和卖出一股股票），但 **股票卖出后需要有一天的冷却时间，即无法进行购买股票**，且必须在再次购买之前卖出股票。
...
* 状态A（s=0）：持有股票状态，可卖出或不交易，StateA[i] = max(StateA[i-1]，StateC[i-1] + prices[i])
* 状态B（s=1）：前一天卖出股票，进入交易休息状态，StateB[i] =StateA[i-1] - prices[i]
* 状态C（s=2）：休息状态，可买入股票，StateC[i] = max(StateC[i-1]，StateB[i-1])

最后，交易结束后可获得的最大利润为 **max(StateB[n-1], StateC[n-1])**

<div align="center"><img src="imgs/shell_stock_problem_1.png" alt="shell_stock_problem_1" style="zoom:100%;" /></div>

```go
func maxProfit(prices []int) int {
    n := len(prices)

    StateA := make([]int, n) // 持有股票, 可卖出状态
    StateB := make([]int, n) // 仅卖出股票，进入冷切期状态
    StateC := make([]int, n) // 不交易, 可买入状态

    StateA[0] = -prices[0]
    for i:=1; i<n; i++ {
        StateA[i] = max(StateA[i-1], StateC[i-1] - prices[i])
        StateB[i] = StateA[i-1] + prices[i]
        StateC[i] = max(StateC[i-1], StateB[i-1])
    }

    return max(StateB[n-1], StateC[n-1])
}
```
...
对于每一天，有三种交易状态buy / sell / cooldown, 且sell 和 cooldown 可以合并成一种状态，因为手里最终没有股票。最终需要的结果是 sell，即手里股票卖了获得最大利润。因此，可以用两个数组来记录当前持股和未持股的状态，令`sell[i]` 表示第i天未持股时，获得的最大利润，`buy[i]`表示第i天持有股票时，获得的最大利润。
...
最终我们要求的结果是`sell[n - 1]`，表示最后一天结束时，手里没有股票时的最大利润，其次由于`sell[i]`仅仅依赖前一项，`buy[i]`仅仅依赖前两项，算法可以优化到`O(1)`。
...
func maxProfit(prices []int) int {
    ...
    for i:=1; i<n; i++ {
        ...
        buy[i] = max(buy[i-1], preSell - prices[i])
...
func maxProfit(prices []int) int {
    ...
    // sell[i], sell[i-2], buy[i]
    currSell, preSell, buy := 0, 0, -prices[0]
    for i:=1; i<n; i++ {
        tmp := currSell
        currSell = max(currSell, buy + prices[i])
        if i == 1 {
...
### 3.8.2 [需要交易费用的股票交易](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-with-transaction-fee/description/)

**问题**：给定一个数组 prices，即  prices[i] 是给定股票第 i 天的价格，以及一个代表交易费用的整数 fee，返回可以获得的最大利润，其中交易数量不限制，但需要为每笔卖出交易支付交易费用，且必须在再次购买之前卖出股票。
...
即有三种交易状态buy / sell / cooldown, 且sell 和 cooldown 可以合并成一种状态，因为手里最终没有股票。最终需要的结果是 sell，即手里股票卖了获得最大利润。因此，可以用两个数组来记录当前持股和未持股的状态：
...
对于`buy[i]`，最大利润有两种可能，一是今天没动作跟昨天持股状态一样，二是昨天天卖了股票，今天买了股票，因为 限制再次买入股票前必须卖出股票。状态转移方程如下：
...
func maxProfit(prices []int, fee int) int {
    n := len(prices)
    // sell[i], buy[i], sell[i-1]
    currSell, currBuy, preSell := 0, 0, 0

    currBuy = -prices[0]
    for i:=1; i<n; i++ {
        preSell = currSell
        // max{sell[i - 1], buy[i-1] + prices[i] - fee}
        currSell = max(currSell, currBuy + prices[i] - fee)
        // max{buy[i-1], sell[i-1] - prices[i]}
...
### 3.8.3 [至多1次股票交易](https://leetcode.com/problems/best-time-to-buy-and-sell-stock/description/)

**问题**：给定一个数组 prices，其中 prices[i] 是给定股票第 i 天的价格，交易周期内选择一天购买一只股票，并选择未来的另一天出售该股票来最大化交易利润（要求：仅能交易一次，且股票卖出前需购买股票）。
...
即只要记录前面的最小价格，将这个最小价格作为买入价格，然后将当前的价格作为售出价格，查看当前收益是不是最大收益。

<div align="center"><img src="imgs/shell_stock_problem_onlyone.png" alt="shell_stock_problem_onlyone" style="zoom:100%;" /></div>
...
对于`buy[i]`，最大利润有两种可能，一是今天没动作跟昨天持股状态一样，二是今天才买入股票（**限制仅能交易一次**）。状态转移方程如下：
...
func maxProfit(prices []int) int {
    ...
    currBuy = -prices[0]
    for i:=1; i<n; i++ {
        curSell = max(curSell, currBuy + prices[i])
        // 仅交易一次
...
### 3.8.4 [至多两次股票交易](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-iii/description/)

**问题**：给定一个数组 prices，其中 prices[i] 是给定股票第 i 天的价格，返回至多k次交易可获取的最大利润（要求：至多交易k次，且股票卖出前需购买股票）。
...
定义 dp[i]\[j] 表示**最多交易i次时在第j天能获得的最大利润**，即 **dp[i]\[j] = max(不做处理， 卖出股票)**

- 如果不做处理，第j天的最大利润就和第j-1天相等；
- 如果卖出股票，设是在第 m 天买入，那么第j天的最大利润就是 **两天的价格差 + 最多交易 i-1 次时第m天的获利**（PS：这里的m需要从0遍历到 j-1）；

<div align="center"><img src="imgs/shell_stock_problem_two_math.png" alt="shell_stock_problem_two_math" style="zoom:100%;" /></div>

**时间优化**

由上述可知在计算 dp[i]\[j] 和 dp[i]\[j+1] 时，向前遍历寻找最合适的买入天（即m）的工作是大部分重合的。如果在遍历 dp 数组中实时更新一个变量maxDiff（表示最多交易 i-1 次时，从第 0 天到第 j-1 天 **最大利润 - 当天价格** 的最大值），就能节省这部分的时间开销。

<div align="center"><img src="imgs/shell_stock_problem_two_math_2.png" alt="shell_stock_problem_two_math_2" style="zoom:100%;" /></div>
...
func maxProfit(prices []int) int {
    ...
    for i:=1; i<=k; i++ {
        preMaxProfit := dp[i-1][0] - prices[0]
        for j:=1; j<n; j++ {
            dp[i][j] = max(dp[i][j-1], prices[j] + preMaxProfit)
            // 表示最多交易i-1次时，从第0天到第j-1天 "最大利润-当天价格" 的最大值
            preMaxProfit = max(preMaxProfit, dp[i-1][j] - prices[j])
        }
    }
...
### 3.8.5 [至多k次股票交易](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-iv/description/)
...
// 可以进行无限次交易
func maxMoreProfit(prices []int) int {
    maxProfit := 0
...
可进行**[无限次股票交易](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-ii/description/)**问题解题思路

例如：[7, 1, 5, 3, 6, 4]

<div align="center"><img src="imgs/shell_stock_problem_4.png" alt="shell_stock_problem_4" style="zoom:100%;" /></div>

从上图可以看到，交易最大利润等于**连续的波谷和波峰差值之和**，即

<div align="center"><img src="imgs/shell_stock_problem_5.png" alt="shell_stock_problem_5" style="zoom:100%;" /></div>

其次，如下图 prices = [1, 7, 2, 3, 6, 7, 6, 7]

<div align="center"><img src="imgs/shell_stock_problem_3.png" alt="shell_stock_problem_3" style="zoom:100%;" /></div>

从上图中，可以观察到 **A + B + C == D**，对应于连续峰谷高度的差值，因此问题可转换为求解数组内连续相邻元素正差值之和。

> PS：因为可以无限次买入和卖出，而且都知道炒股想挣钱当然是 **低价买入高价抛出**，那么这里只需要从第二天开始，如果当前价格比之前价格高，则把差值加入利润中。因为可以昨天买入，今日卖出，若明日价更高的话，还可以今日买入，明日再抛出。以此类推，遍历完整个数组后即可求得最大利润。

​         

## 3.9 字符串编辑

### 3.9.1 [删除最少字符使两个字符串相等](https://leetcode.com/problems/delete-operation-for-two-strings/description/)

**问题**：给定两个字符串 word1 和 word2，返回使 word1 和 word2 相同所需的最少步骤数，其中每一步可以删除任一字符串中的一个字符
...
1）可以转换为求两个字符串的最长公共子序列问题，即 **minDel = len(w1) + len(w2) - 2*lcs(w1, w2)**
...
func minDistance(word1 string, word2 string) int {
    ...
    return n1 + n2 - 2*lcs(word1, word2, n1, n2, memo)
}

func lcs(word1 string, word2 string, n1 int, n2 int, memo [][]int) int {
    if n1 == 0 || n2 == 0 {
        return 0
    }
    ...
    if word1[n1-1] == word2[n2-1] {
        memo[n1][n2] = 1 + lcs(word1, word2, n1-1, n2-1, memo)
    } else {
        memo[n1][n2] = max(lcs(word1, word2, n1-1, n2, memo), lcs(word1, word2, n1, n2-1, memo))
    }
...
即定义 dp[i]\[j] 表示**使 S1 的前 i 个字符与 S2 的前 j 个字符相等最少字符删除次数**

考虑 S1[i] 与 S2[j] 值是否相等，分为两种情况：

- 当 **S1[i]==S2[j]** 时，即无需删除任一字符串的字符使其相等，那么删除次数 **dp[i]\[j] = dp[i-1]\[j-1]**；
- 当 **S1[i] != S2[j]** 时，此时为使S1前i-1个字符子序列 和 S2前 j 个字符子序列相等，则需要删除其中任一子序列，并取最小值+1即可，即 **dp[i]\[j] = min{ dp[i-1]\[j], dp[i]\[j-1] } + 1**；

<div align="center"><img src="imgs/dp_mini_delete_char.png" alt="dp_mini_delete_char" style="zoom:100%;" /></div>

<div align="center"><img src="imgs/dp_mini_delete_char_2.png" alt="dp_mini_delete_char_2" style="zoom:100%;" /></div>
...
### 3.9.2 [字符串间编辑距离](https://leetcode.com/problems/edit-distance/description/)

**问题**：修改一个字符串word1成为另一个字符串word2，使得修改次数最少其中一次修改操作包括：插入 / 删除 / 替换

> **编辑距离**：计算机科学中使用的一个术语，其中编辑距离是一种字符串度量，一种量化两个字符串（例如单词）彼此之间差异程度的方法，通过计算将一个字符串转换为另一个字符串所需的最少操作数来衡量

<div align="center"><img src="imgs/dp_edit_distance.png" alt="dp_edit_distance" style="zoom:100%;" /></div>
...
dp[i]\[j] 表示 word1前i个字符到word2的前j个字符的转化的最小操作数

根据 word1[i] 与 word2[j] 值是否相等，分为两种情况：

* 若word1[i] == word2[j]，那么只要word1的前i-1个能转换到word2的前j-1个即可， dp[i]\[j] = dp[i-1]\[j-1]；
* 若word1[i] != word2[j]，则考虑以下3种情景，并取最小值即可
  * 插入：即给word1插入一个和word2最后的字母相同的字母，这时word1和word2的最后一个字母就一样了，此时编辑距离等于 **1 + 插入前的word1到word2去掉最后一个字母后的编辑距离**，即 dp[i]\[j - 1] + 1
  * 删除：即删除word1的最后一个字母，此时编辑距离等于 **1 + word1去掉最后一个字母到word2的编辑距离**，即 dp[i - 1]\[j] + 1；
  * 替换：即把word1的最后一个字母替换成word2的最后一个字母，此时编辑距离等于 **1 + word1和word2去掉最后一个字母的编辑距离**，即dp[i - 1]\[j - 1] + 1

例如：word1 = "abc", word2 = "arc"

> 1）字符相等
>
> <div align="center"><img src="imgs/dp_equal_characters.png" alt="dp_equal_characters" style="zoom:100%;" /></div>
>
> 2）字符不相等
>
> <div align="center"><img src="imgs/dp_all_operations.png" alt="dp_all_operations" style="zoom:100%;" /></div>
...
<div align="center"><img src="imgs/edit_distance_tree.png" alt="edit_distance_tree" style="zoom:100%;" /></div>
...
func minDistance(word1 string, word2 string) int {
    ...
    return minDistanceRecur(word1, word2, n1, n2, memo)
}

func minDistanceRecur(word1 string, word2 string, n1 int, n2 int, memo [][]int) int {
    if n1 == 0 {
        return n2
    } else if n2 == 0 {
        return n1
    ...
    if word1[n1-1] == word2[n2-1] {
        memo[n1][n2] = minDistanceRecur(word1, word2, n1-1, n2-1, memo)
    } else {
        memo[n1][n2] = 1 + min(
            minDistanceRecur(word1, word2, n1, n2-1, memo),  // 插入
            minDistanceRecur(word1, word2, n1-1, n2, memo),  // 删除
            minDistanceRecur(word1, word2, n1-1, n2-1, memo),// 替换
        )
    }
...
### 3.9.3 [复制粘贴字符](https://leetcode.com/problems/2-keys-keyboard/description/)

**问题**：给定一个整数 n，返回使字符 “A” 恰好出现在屏幕上 n 次所需的最少操作次数，其中可以在此记事本上执行以下两种操作（当前记事本屏幕上只有一个字符“A”）：

* 全部复制：可以复制屏幕上出现的所有字符（不允许部分复制）
* 粘贴：可以粘贴上次复制的字符
...
例如：当 n = 36 （18\*2 = 3\*3\*2\*2）时，如果想获得 36 个相同的A，可以先获得 18 个 A，然后复制 1 次，粘贴 1 次，于是就有了 36 个 A，即 36 = 18 * 2 的含义，然后再思考怎么得到 18 个A，这就是递归。
...
* 5）因为复制和粘贴都算一次操作，所以上面得到了 3 * 3 * 2 * 2 = 36 个'A'。

因此可得出结论：

* 如果 **n 是素数**的话，只能通过复制 1 次A，然后粘贴 n - 1 次的方式才能得到 n 个A，总操作了 n 次；
* 如果 **n = i * j** 的话，最快得到 n 的方式是先得到 i ，复制 1 次，然后再粘贴 j - 1 次，总的就有了 n 个A，总操作了 **minSteps(i) + 1 + j - 1 = minSteps(i) + j = minSteps(i) + n / i **次；

于是就有了递归的解法，即先让 i 从 2 开始遍历找到 n - 1，判断 i 是不是 n 的因子，如果 i 是 n 的因子，那么总的需要操作 minSteps(i) + n / i次。如果从 2 到 n - 1 没有 n 的因子，那么 n 是个素数，必须操作 n 次。
...
上面已经给了递归的做法，看了 36 个 'A' 的分解之后，可知题目要求的就是 36 的**所有素因子之和**（PS：素因子是指该因子不可再次拆分）。

**为什么非得拆解成 素因子 呢**？

> 因为当一个因子还可以分解成更小的因子的时候，那么分解后的结果会更小
>
...
因此，算法通过求解整数 n 能拆成哪些素因子，因此可让 d 试探是否为因子从 2 开始递增，如果 n 能被 d 整除时，此时的 d 是 n 的一个素因子，求复制粘贴次数的结果中增加 d ；而且如果 d 是个素因子，那么要一次性把 n 中除掉所有的 d。

```go
func minSteps(n int) int {
    res, d := 0, 2
    for n > 1 {
        for n%d == 0 { // 此时d是n的一个素因子
            res += d
            n /= d
        }
        d++
...
6. [seramasumi 算法](https://seramasumi.github.io/Algorithms)
7. [Knapsack Pattern — Identify Pattern & Solve with Dynamic Programming](https://itnext.io/identify-knapsack-pattern-solve-with-dynamic-programming-b673bed3324f)
8. [leetcode: an-introduction-to-dynamic-programming](https://leetcode.com/explore/featured/card/dynamic-programming/630/an-introduction-to-dynamic-programming/)
